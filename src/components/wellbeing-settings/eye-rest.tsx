...
export function EyeRest({ settings, lastFiredAt, updateInterval, toggleEnabled }: Props) {
  return (
    <section
    ...
    >
      ...
      <div className="bg-white/60 dark:bg-black/20 backdrop-blur-sm border border-emerald-100/50 dark:border-emerald-900/30 p-6 rounded-lg relative z-10 mt-auto">
        ...
        >
          <label
            className={`flex flex-col items-center justify-center p-4 border-2 rounded-xl cursor-pointer transition-all text-center gap-1 ${settings.breaks.eye.intervalMinutes === 20 ? "border-emerald-500 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400" : "border-transparent bg-card "}`}
          >
          ...
          <label
            className={`flex flex-col items-center justify-center p-4 border-2 rounded-xl cursor-pointer transition-all text-center gap-1 ${settings.breaks.eye.intervalMinutes === 30 ? "border-emerald-500 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400" : "border-transparent bg-card "}`}
          >
