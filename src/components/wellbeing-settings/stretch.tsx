...
export function Stretch({ settings, updateInterval, toggleEnabled }: Props) {
  return (
    <section
      className={`bg-card p-6 rounded-lg shadow-sm border border-emerald-100/30 flex flex-col transition-all duration-300 relative overflow-hidden ${!settings.breaks.stretch.enabled ? "opacity-60 grayscale-[50%] " : ""}`}
    >
      ...
      <div className="bg-white/60 dark:bg-black/20 backdrop-blur-sm border border-emerald-100/50 dark:border-emerald-900/30 p-6 rounded-lg relative z-10 mt-auto">
        ...
        >
          {[60, 90, 120].map((mins) => (
            <label
              key={mins}
              className={`flex flex-col items-center justify-center p-4 border-2 rounded-xl cursor-pointer transition-all text-center ${settings.breaks.stretch.intervalMinutes === mins ? "border-emerald-500 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 text-primary" : "border-transparent bg-card  text-foreground"}`}
            >
